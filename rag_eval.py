...
import anthropic
from rag_basic import chunks_list, hybrid_search
...
EVAL_SET_CHUNKS_BY_SECTION = [
    {"question": 'What are the yield sources of the “Flagship USDC SuperVaul?”',
    ...
    {"question": 'How many days does it take to unstake sUP rewards of the “Flagship USDC SuperVault”',
        "must_contain": [["14-day", "14 days", "fourteen days"]]},
    {"question": 'What are the three stable coins that are allowed to provide liquidity to the pool in the “wnAUSD-wnUSDC-wnUSDT0”',
        "must_contain": [["USDT0"], ["AUSD"], ["USDC"]]},
    {"question": 'When will you know you have finished setting up the “wnAUSD-wnUSDC-wnUSDT0”?',
        "must_contain": [["BPT", "Balancer Pool Tokens"]]},
    {"question": 'Who did the latest audit for Balancer?',
        "must_contain": [["Centora"]]},
    {"question": 'What are the yield sources of the “wnAUSD-wnUSDC-wnUSDT0?”',
        "must_contain": [["Swap fees"], ["Yield-bearing stablecoins", "yield bearing stablecoins"], ["Merkl incentives", "Merkl"]]},
    {"question": 'How much yield does Merlk incentives contribute to the “wnAUSD-wnUSDC-wnUSDT0”?',
...
for i in range (0, len(EVAL_SET_CHUNKS_BY_SECTION)):
    ...
    sorted_chunks = hybrid_search(question)

    key_found_infor = []
    missing_keys = []

    for key in must_contain_clean:
        for num in range(0, len(sorted_chunks)):
            idx = sorted_chunks[num]["chunk_infor_indice"]
            check = any(k in chunks_clean[idx] for k in key)
            if check:
